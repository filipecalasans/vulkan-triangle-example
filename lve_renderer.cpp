...
LveRenderer::LveRenderer(LveNativeWindow& window, LveDevice& device) : lveWindow(window), lveDevice(device) {
  recreateSwapChain();
...
void LveRenderer::endFrame() {
    assert(isFrameStarted && "Can't end a frame that hasn't started yet.");
    auto commandBuffer = getCurrentCommandBuffer();
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR || result == VK_SUBOPTIMAL_KHR || lveWindow.resized()) {
        lveWindow.resetResized();
        recreateSwapChain();
